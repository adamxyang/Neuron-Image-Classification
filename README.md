...
We will train models using images from 33 plates and test the model on the rest of the three plates. To train the classifier, we only use 0 and 30 μM of Aβ with 0 μM candidate compound. Then we apply the model to test whether candidate drugs have any effect. The plate configuration is shown below.
